...
pub const QuantumSubgraph = struct {
    ...
    pub fn init(allocator: Allocator) Self {
        return Self{
            ...
            .subgraph_id = @as(u64, @bitCast(std.time.nanoTimestamp())),
            .allocator = allocator,
        };
    }
...
test "quantum_task_adapter_identify_subgraphs" {
    ...
    var n1 = try Node.init(allocator, "n1", "data1", 0.7, 0.7, 0.5);
    try graph.addNode(n1);
    var n2 = try Node.init(allocator, "n2", "data2", 0.6, 0.8, 0.3);
    try graph.addNode(n2);

    var e1 = try Edge.init(allocator, "n1", "n2", .entangled, 0.9, 0.8, 0.8, 2.0);
