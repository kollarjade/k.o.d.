...
pub const Term = struct {
    ...
    pub fn computeHash(self: *const Self, hasher: *Sha256) void {
        ...
        std.mem.writeInt(u64, &len_buf, self.args.items.len, .Little);
...
pub const Proposition = struct {
    ...
    pub fn computeHash(self: *Self) [32]u8 {
        ...
        var len_buf: [8]u8 = undefined;
        std.mem.writeInt(u64, &len_buf, self.terms.items.len, .Little);
        hasher.update(&len_buf);
        for (self.terms.items) |term| {
            term.computeHash(&hasher);
        }
        std.mem.writeInt(u64, &len_buf, self.sub_propositions.items.len, .Little);
        hasher.update(&len_buf);
        for (self.sub_propositions.items) |sub| {
...
pub const HoareLogicVerifier = struct {
    ...
    pub fn composeTriples(self: *Self, ids: []const u64) !u64 {
        ...
        const op_name = std.fmt.bufPrint(&op_buf, "seq_{d}_{d}", .{ ids[0], ids[ids.len - 1] }) catch "seq";
...
pub const FormalVerificationEngine = struct {
    ...
    pub fn verifyGraph(self: *Self, graph: *const SelfSimilarRelationalGraph) !*VerificationResult {
        ...
        std.mem.writeInt(u64, &node_count_buf, graph.nodes.count(), .Little);
        hasher.update(&node_count_buf);
        var edge_count_buf: [8]u8 = undefined;
        std.mem.writeInt(u64, &edge_count_buf, graph.edges.count(), .Little);
        hasher.update(&edge_count_buf);
        ...
        const exec_time: i64 = end_time - start_time;
    ...
    pub fn getStatistics(self: *const Self) EngineStatistics {
        ...
        return EngineStatistics{
            ...
            .uptime_ms = current_time - self.creation_time,
        };
    }
