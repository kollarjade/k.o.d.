...
pub const SignalPropagationEngine = struct {
    ...
    pub fn initiateSignal(self: *Self, source_node_id: []const u8, initial_signal: SignalState) !void {
        ...
        if (amp > 0.0) {
            const current_mag = source_node.?.quantum_state.magnitude();
            const scale = amp / @max(current_mag, 0.01);
            source_node.?.quantum_state.re *= scale;
            source_node.?.quantum_state.im *= scale;
        }
    }
    ...
    pub fn propagateStep(self: *Self) !void {
        ...
        var node_signals = AutoHashMap([]const u8, SignalState).init(self.allocator);
        defer {
        ...
        while (edge_iter.next()) |entry| {
            ...
            const source_node = self.graph.getNode(source_id) orelse continue;

            const source_signal = SignalState.init(
                source_node.quantum_state.magnitude(),
                source_node.phase,
                1.0,
            );
            ...
            for (entry.value_ptr.items) |edge| {
                ...
                _ = self.graph.getNode(target_id) orelse continue;
                ...
                transmitted_signal.phase += edge.quantum_correlation.arg();
        ...
        while (signal_iter.next()) |entry| {
            ...
            const current_mag = node.quantum_state.magnitude();
            const new_mag = complex_rep.magnitude();
            if (new_mag > 0.0 and current_mag > 0.0) {
                const blend = 0.7;
                const final_mag = blend * new_mag + (1.0 - blend) * current_mag;
                node.quantum_state.re = complex_rep.re / new_mag * final_mag;
                node.quantum_state.im = complex_rep.im / new_mag * final_mag;
            }
            ...
            try self.flow_analyzer.recordAccess(
                std.mem.asBytes(&std.mem.hash_map.hashString(node_id))[0..16].*,
                0,
            );
        }
...
test "signal_propagation_basic" {
    ...
    var n2 = try Node.init(allocator, "n2", "data2", 0.0, 1.0, 0.0);
    try graph.addNode(n2);

    var e1 = try Edge.init(allocator, "n1", "n2", .coherent, 0.9, 0.5, 0.5, 1.2);
