...
pub const ExecutionHistoryEntry = struct {
    ...
    pub fn clone(self: *const Self, allocator: Allocator) !Self {
        var new_entry = Self{
            .action = self.action,
            .primary_target = try allocator.dupe(u8, self.primary_target),
            .secondary_targets = ArrayList([]const u8).init(allocator),
            .operation_type = null,
            .result_value = null,
...
pub const ZVariable = struct {
    ...
    pub fn assign(self: *Self, value: []const u8) !void {
        ...
        const history_entry = try HistoryEntry.init(self.allocator, .assign, value);
        try self.history.append(history_entry);
    }

    pub fn getValue(self: *const Self) ?[]const u8 {
        if (self.graph.nodeCount() == 0) {
            return null;
        }

        var node_iter = self.graph.nodes.iterator();
        var latest_node_id: ?[]const u8 = null;
        while (node_iter.next()) |entry| {
            latest_node_id = entry.key_ptr.*;
        }

        if (latest_node_id) |node_id| {
            return self.graph.decodeInformation(node_id);
        }
    ...
    pub fn relateTo(self: *Self, other: *Self, relationship_type: EdgeQuality) !void {
        if (self.graph.nodeCount() == 0 or other.graph.nodeCount() == 0) {
            return;
        }

        const self_node_id = self.getLatestNodeId() orelse return;
        const other_node_id = other.getLatestNodeId() orelse return;

        const self_state = self.graph.getQuantumState(self_node_id) orelse Complex(f64).init(0.0, 0.0);
        const other_state = other.graph.getQuantumState(other_node_id) orelse Complex(f64).init(0.0, 0.0);
        const other_conj = other_state.conjugate();
        const correlation = self_state.mul(other_conj);

        const weight = correlation.magnitude();
        const fractal_dim = self.graph.calculateFractalDimension();
        ...
        try self.graph.addEdge(edge);

        if (self.logic.stateCount() > 0 and other.logic.stateCount() > 0) {
            try self.logic.entangle(self.logic.stateCount() - 1, 0);
        }

        var buf: [128]u8 = undefined;
        const relate_str = std.fmt.bufPrint(&buf, "relate:{s}:{s}", .{ self_node_id, other_node_id }) catch "relate";
        const history_entry = try HistoryEntry.init(self.allocator, .relate, relate_str);
        try self.history.append(history_entry);
    }
    ...
    pub fn transform(self: *Self, gate: LogicGate, params: ?[]const f64) !void {
        ...
        const history_entry = try HistoryEntry.init(self.allocator, .transform, gate.toString());
        try self.history.append(history_entry);
    }
    ...
    pub fn measure(self: *Self) MeasurementResult {
        if (self.logic.stateCount() > 0) {
            ...
            const history_entry = HistoryEntry.init(self.allocator, .measure, measure_str) catch {
                return result;
            };
    ...
    pub fn getFractalDimension(self: *Self) f64 {
        return self.graph.calculateFractalDimension();
    }
...
pub const RelationalOperationType = enum(u8) {
    ...
    pub fn fromString(s: []const u8) ?RelationalOperationType {
        const lower = s;
        if (std.mem.eql(u8, lower, "and") or std.mem.eql(u8, lower, "AND")) return .op_and;
        if (std.mem.eql(u8, lower, "or") or std.mem.eql(u8, lower, "OR")) return .op_or;
        if (std.mem.eql(u8, lower, "xor") or std.mem.eql(u8, lower, "XOR")) return .op_xor;
        if (std.mem.eql(u8, lower, "entangle") or std.mem.eql(u8, lower, "ENTANGLE")) return .op_entangle;
        return null;
    }
...
pub const ZRuntime = struct {
    ...
    pub fn init(allocator: Allocator) !*Self {
        ...
        global_graph.* = SelfSimilarRelationalGraph.init(allocator);
    ...
    pub fn getVariableConst(self: *const Self, name: []const u8) ?*ZVariable {
        return self.variables.get(name);
    }
    ...
    pub fn propagateInformation(self: *Self, source_var_name: []const u8, depth: i64) !ArrayList([]const u8) {
    ...
    pub fn getSystemState(self: *Self) !SystemState {
        ...
        while (var_iter.next()) |entry| {
            ...
            const var_state = VariableState{
                ...
                .value = if (current_value) |v| try self.allocator.dupe(u8, v) else null,
                .node_count = node_count,
                .edge_count = edge_count,
                .fractal_dimension = fractal_dim,
                .topology_hash = try self.allocator.dupe(u8, topo_hash),
    ...
    pub fn executeQuantumCircuit(self: *Self, var_name: []const u8, circuit: []const GateSpec) !bool {
        ...
        for (circuit) |spec| {
            ...
            variable.logic.applyGate(gate, spec.indices, spec.params) catch continue;
        }
