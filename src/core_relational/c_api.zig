...
const FLOAT_EPSILON: f64 = std.math.f64_epsilon;
    ...
    fn computeEnergy(graph: *const SelfSimilarRelationalGraph) f64 {
        ...
        while (edge_iter.next()) |entry| {
            for (entry.value_ptr.items) |edge| {
                ...
                const m = edge.quantum_correlation.magnitude();
                acc.add(m);
            }
        }
        var node_iter = graph.nodes.iterator();
        while (node_iter.next()) |entry| {
            const m = entry.value_ptr.quantum_state.magnitude();
            acc.add(m);
        }
        return acc.sum;
    }
    ...
    fn rollbackPerturbation(graph: *SelfSimilarRelationalGraph, records: []const PerturbationRecord) void {
        for (records) |rec| {
            if (rec.type == .Edge) {
                ...
                if (graph.edges.get(key)) |list| {
                    if (list.items.len > 0) {
                        list.items[0].weight = rec.old_weight;
                    }
                }
            } else if (rec.type == .Node) {
                if (graph.nodes.get(rec.target_id)) |node| {
                    node.quantum_state.re = rec.old_real;
                    node.quantum_state.im = rec.old_imag;
                }
            }
        }
    }
    ...
    pub fn optimize(self: *Self, graph_ctx: *GraphContext) !void {
        ...
        while (iteration < self.max_iterations) : (iteration += 1) {
            ...
            if (self.acceptMove(delta_energy)) {
                ...
                stagnation_counter += 1;
                self.rollbackPerturbation(graph, perturbation_records.items);
            }
            self.statistics.current_energy = current_energy;
            self.statistics.temperature = self.temperature;
...
export fn jaide_create_graph() callconv(.C) ?*CGraph {
    ...
    ctx.* = GraphContext{
        .inner = SelfSimilarRelationalGraph.init(allocator),
        .lock = .{},
        .allocator = allocator,
    };
...
export fn jaide_add_node(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    if (ctx.inner.hasNode(id_slice)) return JAIDE_ERROR_DUPLICATE_NODE;
    var node = Node.init(ctx.allocator, id_slice, type_slice, 1.0, 0.0, 0.0) catch return JAIDE_ERROR_OUT_OF_MEMORY;
...
export fn jaide_remove_node(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    if (!ctx.inner.hasNode(id_slice)) return JAIDE_ERROR_NODE_NOT_FOUND;
    ...
    for (edges_to_remove.items) |key| {
        if (ctx.inner.edges.fetchRemove(key)) |removed| {
            for (removed.value.items) |*edge| edge.deinit();
            removed.value.deinit();
        }
    }
    if (ctx.inner.nodes.fetchRemove(id_slice)) |removed| {
        removed.value.deinit();
    }
...
export fn jaide_set_node_quantum_state(
    ...
    const q = if (mag2 > 0 and std.math.isFinite(mag2)) 
        Complex(f64).init(real / @sqrt(mag2), imag / @sqrt(mag2))
    else 
        Complex(f64).init(1.0, 0.0);
    ctx.inner.setQuantumState(id_slice, q) catch return JAIDE_ERROR_OPERATION_FAILED;
    return JAIDE_SUCCESS;
}
...
export fn jaide_get_node_quantum_state(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const node = ctx.inner.getNode(id_slice) orelse return JAIDE_ERROR_NODE_NOT_FOUND;
...
export fn jaide_apply_gate(
    ...
    defer ctx.lock.unlock();
    if (!ctx.inner.hasNode(id_slice)) return JAIDE_ERROR_NODE_NOT_FOUND;
    const gate_func = switch (gate_type) {
        0 => nsir_core.identityGate,
        1 => nsir_core.hadamardGate,
        2 => nsir_core.pauliXGate,
        3 => nsir_core.pauliYGate,
        4 => nsir_core.pauliZGate,
        else => return JAIDE_ERROR_UNKNOWN_GATE,
    };
    ctx.inner.applyQuantumGate(id_slice, gate_func) catch return JAIDE_ERROR_MATH_ERROR;
    return JAIDE_SUCCESS;
}
...
export fn jaide_add_edge(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    if (!ctx.inner.hasNode(source_slice) or !ctx.inner.hasNode(target_slice)) return JAIDE_ERROR_NODE_NOT_FOUND;
    ...
    var edge = Edge.init(
        ...
        0.0,
        1.0,
    ) catch return JAIDE_ERROR_OUT_OF_MEMORY;
    ctx.inner.addEdge(edge) catch {
        edge.deinit();
        return JAIDE_ERROR_OPERATION_FAILED;
    };
    return JAIDE_SUCCESS;
...
export fn jaide_get_edge_weight(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const edges = ctx.inner.getEdgeList(source_slice, target_slice);
...
export fn jaide_set_edge_weight(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const edges = ctx.inner.getEdges(source_slice, target_slice) orelse return JAIDE_ERROR_EDGE_NOT_FOUND;
...
export fn jaide_entangle_nodes(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    if (!ctx.inner.hasNode(n1) or !ctx.inner.hasNode(n2)) return JAIDE_ERROR_NODE_NOT_FOUND;
...
export fn jaide_decode_information(
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const data = ctx.inner.decodeInformation(ctx.allocator, id_slice) orelse return JAIDE_ERROR_NODE_NOT_FOUND;
...
export fn jaide_get_topology_hash(
    ...
    defer ctx.allocator.free(hash_hex);
...
export fn jaide_measure_node(graph: ?*CGraph, node_id: [*c]const u8) callconv(.C) f64 {
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    if (!ctx.inner.hasNode(id_slice)) return -1.0;
    const v = ctx.inner.measure(id_slice);
    return clamp01(v);
}
...
export fn jaide_get_node_probability(graph: ?*CGraph, id: [*c]const u8) callconv(.C) f64 {
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const node = ctx.inner.getNode(id_slice) orelse return -1.0;
    const re = node.quantum_state.re;
    const im = node.quantum_state.im;
    const p = re * re + im * im;
    return clamp01(p);
}
...
export fn jaide_has_node(graph: ?*CGraph, id: [*c]const u8) callconv(.C) c_int {
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    return @intFromBool(ctx.inner.hasNode(id_slice));
}
...
export fn jaide_has_edge(graph: ?*CGraph, source: [*c]const u8, target: [*c]const u8) callconv(.C) c_int {
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    return @intFromBool(ctx.inner.hasEdge(s, t));
}
...
export fn jaide_get_edge_fractal_dimension(
    ...
    const edges = ctx.inner.getEdgeList(source_slice, target_slice);
    if (edges.len == 0) return -1.0;
    const d = edges[0].fractal_dimension;
...
export fn jaide_get_fractal_dimension(graph: ?*CGraph) callconv(.C) f64 {
    ...
    ctx.lock.lock();
    defer ctx.lock.unlock();
    const v = ctx.inner.calculateFractalDimension();
...
export fn jaide_get_node_magnitude(
    ...
    defer ctx.lock.unlock();
    const node = ctx.inner.getNode(id_slice) orelse return -1.0;
    const m = node.quantum_state.magnitude();
...
export fn jaide_get_edge_correlation_magnitude(
    ...
    const edges = ctx.inner.getEdgeList(source_slice, target_slice);
    if (edges.len == 0) return -1.0;
    const v = edges[0].quantum_correlation.magnitude();
